...
def create_matrix_with_text_patterns(text_lines, target_text="AutoSuper", output_file='matrix_with_text.mp4', duration=10, fps=30):
    ...
    # Define positions for target text characters
    target_positions = []
    for i in range(target_len):
        # Randomly choose a column and row for each character
        col = random.randint(0, cols - 1)
        row = random.randint(0, height // font_size - 1)
    ...
    for frame_idx in range(total_frames):
        ...
        # Draw target text characters
        for i, (col, row) in enumerate(target_positions):
            x = col * font_size
            y = row * font_size
            char = target_chars[i]
            cv2.putText(img, char, (x, y), font, font_scale, (0, 255, 255), thickness)  # Yellow
    ...
    # Release everything
    out.release()
    print(f"Matrix effect with prominent text saved as {output_file}")
...
# The rest of the functions are already defined above
    """
                    ...
                    if not is_in_pattern:
                        ...
                        if (outline_start_col <= i < outline_end_col and 
                            ...
                            for r in range(max(0, row_in_pattern-1), min(char_height, row_in_pattern+2)):
                                for c in range(max(0, (i-font_size//2) - char_start_col), 
                                              min(char_width, (i+font_size//2) - char_start_col + 1)):
...
def create_matrix_with_enhanced_text(target_text="AutoSuper", output_file='matrix_enhanced_text.mp4', duration=10, fps=30):
    """
